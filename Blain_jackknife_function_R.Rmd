...
gamma.jackknife <- function(a_vector) {
  matrix_gamma = matrix(NA, nrow=2, ncol = length(a_vector))
  for(i in seq_along(a_vector)) {
  matrix_gamma[,i] = gamma.est(a_vector[-i])
  }
  
  variancea = var(matrix_gamma[1,])
  variances = var(matrix_gamma[2,])
  weight <- ((length(a_vector) - 1) ^ 2)/length(a_vector)
  weighted_variancea <- variancea * weight
  weighted_variances <- variances * weight
  jackknife.stderrs <- c(a = sqrt(weighted_variancea), s =sqrt(weighted_variances)) 
...
jackknife.lm <- function(df,formula,p) {
  ...
  matrix_lm = matrix(NA, p+1, nrow(df))
  for(i in 1:nrow(df)) {
    matrix_lm[,i] = lm(formula, data = df[-i,])$coefficients
  }
  
  variance <- numeric()
  for(i in 1:nrow(matrix_lm)) {
    variance[i] = var(matrix_lm[i,])  
  }

  weight = ((nrow(df) - 1) ^ 2)/nrow(df)
  weighted_variance = variance * weight
  
  jackknife.stderrs <- sqrt(weighted_variance)
...
jackknife.lm(df=data,formula="output~input",p=3)
```
