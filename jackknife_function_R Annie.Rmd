...
var_a=var(a)
jackknife.stderr<-sqrt(var_a*(n-1)^2/n)
...
gamma.jackknife <- function(a_vector) {
  n<-length(a_vector)
  gamma.para<-matrix(NA,nrow=2,ncol=length(a_vector))
 for(i in seq_along(a_vector)){
   gamma.para[,i]<-gamma.est(a_vector[-i])
 }
  weight=(n-1)^2/n
  jackknife.stderrs<-c(var_a=sqrt(var(gamma.para[1,])*weight),var_s=sqrt(var(gamma.para[2,])*weight))
...
jackknife.lm <- function(df,formula,p) {
  ...
  para<-matrix(NA,nrow=p,ncol=n)
for(i in 1:nrow(df)){
  para[,i]<-coefficients(lm(formula,data=df[-i,]))
}
  weight=(n-1)^2/n
  for(i in 1:p){
    jackknife.stderr[i]<-sqrt(var(para[i,])*weight)
  }
  return(jackknife.stderr)
}
```
...
jackknife <- function(estimator,the_data) {
   omit_and_est <- function(omit) {
             est<-estimator(omit.case(the_data,omit))
             return(est)
        }
...
scale_and_sqrt_vars <- function(jackknife.ests,n) {
  var_est<-apply(jackknife.ests,1,var)
  scale_var<-((n-1)^2/n)*var_est
  jackknife.stderr<-sqrt(scale_var)
  return(jackknife.stderr)
}
```
