...
**PHP 2560 Only** (Why $(n-1)^2 / n$?  Think about just getting the standard
    error of the mean)
#This is a weight in order to correct the calculation of the mean as we end up calculating n-1 values after omitting each value once.#
...
gamma.jackknife <- function(a_vector) {
  ...
  gamma_matrix <- matrix(NA, 2, length(a_vector))
  for(i in seq_along(a_vector)) {
    gamma_matrix[,i] <- gamma.est(a_vector[-i])
  }
  
  variance <- c(var(gamma_matrix[1,]), var(gamma_matrix[2,]))
  weight <- ((length(a_vector) - 1) ^ 2)/length(a_vector)
  weighted_variance <- variance * weight
  
  jackknife.stderrs <- sqrt(weighted_variance)
  return(c(a=jackknife.stderrs[1], s=jackknife.stderrs[2]))
}
```
...
jackknife.lm <- function(df,formula,p) {
  ...
  coeffs_matrix <- matrix(NA, p+1, nrow(df))
  for(i in 1:nrow(df)) {
    coeffs_matrix[,i] <- lm(formula, data = df[-i,])$coefficients
  }
  
  variance <- numeric()
  for(i in 1:nrow(coeffs_matrix)) {
    variance[i] <- var(coeffs_matrix[i,])  
  }
