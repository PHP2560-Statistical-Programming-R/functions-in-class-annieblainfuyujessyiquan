...
mean.jackknife <- function(a_vector) {
  ...
  for(i in seq_along(a_vector)){
    mean_vector[i] <- mean(a_vector[-i])
    
    var <- var(mean_vector)
    weight <- ((length(a_vector) - 1) ^ 2)/length(a_vector)
    weighted_variance <- var * weight
  
  jackknife.stderr <- sqrt(weighted_variance)
...
gamma.jackknife <- function(a_vector) {
  gamma.matrix <- matrix(NA, nrow=length(a_vector), ncol=2)
  for(i in seq_along(a_vector)){
  gamma.matrix[i,] <- gamma.est(a_vector[-i])
}
  var_a <- var(gamma.matrix[,1])
  var_s <- var(gamma.matrix[,2])
  
  weight <- (length(a_vector) - 1) ^ 2/length(a_vector)
  
  weighted_a <- var_a * weight
  weighted_s <- var_s * weight
  
  jackknife.stderrs <- c(a = sqrt(weighted_a), s = sqrt(weighted_s))
...
jackknife.lm <- function(df,formula,p) {
  ...
  coeffs_mat <- matrix(NA, p, nrow(df))
  
  for(i in 1:nrow(df)){
  coeffs_mat[,i] <- lm(formula, data = df[-i,])$coefficients
  }
  
 variance <- numeric()
 for(i in 1:nrow(coeffs_mat)){
   variance[i] <- var(coeffs_mat[i,])
 }
 
 weight <- ((nrow(df) - 1) ^ 2)/nrow(df)
 
 jackknife.stderr <- sqrt(variance*weight)
...
- Write a function `omit.case` that omits a point given the data and returns the data minus that point. Make sure it can handle higher dimensions. 
    ```{r, eval=FALSE}
          omit.case <- function(the_data,omitted_point) {
            ...
            if (is.null(data_dim)||length(the_data)==1 ){
...
- Write a function `omit_and_est` that takes the data with an omitted point and returns whatever function your estimator does.  
    ```{r,eval=FALSE}
    omit_and_est <- function(estimator,omit) {
...
jackknife <- function(estimator, the_data){
  data_dim <- dim(the_data)
    ...
    weight <- (n-1)^2/n
    jackknife.var <- weight*var.of.reestimates
