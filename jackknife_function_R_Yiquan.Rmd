...
gamma.jackknife <- function(a_vector) {
   gamma.matrix <- matrix(NA,length(a_vector),2)
   for(i in seq_along(a_vector)){
    gamma.matrix[i,]<-gamma.est(a_vector[-i])
}
   variancea=var(gamma.matrix[,1])
   variances=var(gamma.matrix[,2])
   weight=((length(a_vector)-1)^2)/length(a_vector)
   weighta<-variancea*weight
   weights<-variances*weight
   jackknife.stderrs<-c(a=sqrt(weighta),s=sqrt(weights))
...
jackknife.lm <- function(df,formula,p) {
  n<-nrow(df)
  jackknife.stderr<-vector("double",p)
  para<-matrix(NA,nrow=p,n)
  for(i in 1:n){
    para[,i]<-lm(formula,data=df[-i,])$coefficient
  }
  weight<-(n-1)^2/n
  for(i in 1:p){
    jackknife.stderr[i]<-sqrt(weight*var(para[i,]))
  }
 return(jackknife.stderr)
}
```
...
```{r,eval=FALSE}
       ...
       omit_case <- function(the_data,omitted_point) {
         if(is.null(dim(the_data))){
             n<- the_data[-omitted_point]
            } else{
             n<- the_data[-omitted_point,]
            }
       }
...
jackknife <- function(estimator,the_data) {
  omit_and_est<-function(omit){
    est <- estimator(omit_case(the_data,omit))
  }
  # Define n to be lenth or number of rows here
  n<-select.dim(the_data)
